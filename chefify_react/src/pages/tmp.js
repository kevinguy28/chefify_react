...
import { getSteps, getMessages, getRecipeComponents, postMessage, getRecipe, postReview, putReview, deleteReview, getUser, getUserReview} from '../utils/CRUD';
...
const RecipePage = () => {
  ...
  const userReview = () =>{
    if(load === false){
      if(Object.keys(getReview()).length > 0){ /* Check if user left a review for this recipe*/
        const review = getReview();
  ...
  const getReview = () =>{ /* Checks if user left a review for this recipe and returns it*/ 
    if(userData.reviews){
      for(const review of userData.reviews){
        if(recipe.id === review.recipe.id){
          return(review);
        };
      };
      return({})
  ...
  let fetchData = async () => {
    if(load){
      ...
      let userData = await getUser(authTokens, user.user_id, csrfToken);
      setSteps(stepsData);
  ...
  let fetchReview = async () =>{
    if(!load){
      let csrfToken = getCsrfToken();
      let {reviewUser} = await getUserReview(authTokens, recipeId, user.user_id, csrfToken);
  ...
  },[load, steps, messages, editOnMode, editOffMode, submitMode])
